...
class CommonController extends Controller
{
    ...
    public static  function post_hongbao(Request $request, Thread $thread, Post $post)
    {
        ...
        } elseif (Carbon::now() > Carbon::create("2026-2-19 0:0:0")) {
            $coin = 0;
            ...
            switch ($rand_num) {
                case $rand_num == 1000: {
                        if (DB::table("hongbao_record")->where('rand_num', 1000)->exists()) {
                            ...
                            $coin = 100000;
                            $message = "这是岛主埋单唯一大礼10w olo！马上截图去楼里炫耀吧！！";
                            break;
                        }
                    }
                case $rand_num >= 990: {
    ...
    public function store_hongbao_pool(Request $request)
    {
        ...
        if (Carbon::now() < Carbon::create("2026-2-17 0:0:0") && $user->admin != 99) {
            return response()->json([
                'code' => ResponseCode::USER_CANNOT,
                'message' => "本次活动尚未开始，请稍等喔",
            ]);
        }
        if (Carbon::now() > Carbon::create("2026-2-19 0:0:0")) {
            return response()->json([
                'code' => ResponseCode::USER_CANNOT,
                'message' => "本次活动已经结束，你来晚啦",
            ]);
        }
        if (Carbon::parse($user->created_at) > Carbon::create("2026-2-2 0:0:0")) {
            return response()->json([
                'code' => ResponseCode::USER_CANNOT,
                'message' => "你的饼干不符合参与条件（26年2月2日0点前领取的饼干）",
            ]);
        }
